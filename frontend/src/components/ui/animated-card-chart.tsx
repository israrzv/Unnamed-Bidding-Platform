...
const Layer1: React.FC<LayerProps> = ({ color, secondaryColor }) => {
  return (
    <div
      ...
      style={
        { "--color": color, "--secondary-color": secondaryColor } as React.CSSProperties
      }
    >
...
const Layer2: React.FC<{ color: string }> = ({ color }) => {
  return (
    <div className="group relative h-full w-[356px]" style={{ "--color": color } as React.CSSProperties}>
      <div className="absolute inset-0 z-[7] flex w-[356px] translate-y-full items-start justify-center bg-transparent p-4 transition-transform duration-500 ease-[cubic-bezier(0.6,0.6,0,1)] group-hover/animated-card:translate-y-0">
        <div className="rounded-md border border-zinc-200 bg-white/25 p-1.5 opacity-0 backdrop-blur-sm transition-opacity duration-500 group-hover/animated-card:opacity-100 dark:border-zinc-800 dark:bg-black/25">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs text-black dark:text-white">Fair-price clearings</p>
          </div>
          <p className="text-xs text-neutral-500 dark:text-neutral-400">
            What buyers saved versus resale.
          </p>
        </div>
      </div>
    </div>
...
const Layer4: React.FC<LayerProps> = ({ color, secondaryColor, hovered }) => {
  ...
  return (
    <div className="absolute inset-0 z-[8] flex h-[180px] w-[356px] items-center justify-center text-neutral-800/10 transition-transform duration-500 ease-[cubic-bezier(0.6,0.6,0,1)] group-hover/animated-card:scale-150 dark:text-white/15">
